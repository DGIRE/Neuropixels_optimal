"""find_best_unit_fig.py -- NP-DEMO-4: find and plot single best-responding unit.

Identifies the unit whose mean per-trial max firing rate in 10-20 s (odor window)
shows the greatest percent increase over its mean per-trial max firing rate in
0-10 s (control window).  Renders a two-panel waterfall + line figure for that
unit only, all its trials, with experiment ID and unit ID annotated.

Metric per unit:
  For each trial t:
    max_control[t] = max( FR_unit[samples with ts in 0-10 s] )
    max_odor[t]   = max( FR_unit[samples with ts in 10-20 s] )
  pct_increase = 100 * (mean(max_odor) - mean(max_control)) / mean(max_control)
"""
import os
import sys
import numpy as np
...
LV_FS      = 125.0   # LabView sample rate (Hz)
MIN_FONT   = 8
DPI        = 150
COLORMAP   = "jet"
CONTROL_S  = (0.0, 10.0)
...
for sess_dir in SESSIONS:
    ...
    fr_per_unit = np.asarray(D["firing_rate_per_unit"], dtype=np.float64)  # (n_units, n_samp)
    trials = _segment_odd_tr_trials(D)
    print(f"  {len(trials)} trials")
    if not trials:
        continue

    # For each unit accumulate per-trial max in each window
    n_units = len(included_ids)
    unit_max_control = [[] for _ in range(n_units)]  # list-of-lists
    unit_max_odor    = [[] for _ in range(n_units)]
    ...
    for trial in trials:
        ...
        for ui in range(n_units):
            unit_max_control[ui].append(float(np.max(fr_per_unit[ui, ctrl_idx])))
            unit_max_odor[ui].append(float(np.max(fr_per_unit[ui, odor_idx])))

    for ui, uid in enumerate(included_ids):
        mc = unit_max_control[ui]
        mo = unit_max_odor[ui]
        if len(mc) < 2:
            continue
        ...
        if mean_mc <= 0:
            continue
        pct = 100.0 * (mean_mo - mean_mc) / mean_mc
        if pct > best["pct_increase"]:
            best = dict(
                pct_increase=pct,
                exp_date=exp_date,
                unit_id=int(uid),
                unit_idx=ui,
                mean_max_control=mean_mc,
                mean_max_odor=mean_mo,
                # store per-trial FR vectors for the figure
                trials=trials,
                fr_row=fr_per_unit[ui, :].copy(),
            )

    print(f"  Best so far: {best.get('exp_date','?')} unit {best.get('unit_id','?')} "
          f"pct={best['pct_increase']:.1f}%")

print(f"\n{'='*60}")
print(f"BEST UNIT: session={best['exp_date']}, unit_id={best['unit_id']}")
print(f"  mean max FR control (0-10s): {best['mean_max_control']:.2f} Hz")
print(f"  mean max FR odor   (10-20s): {best['mean_max_odor']:.2f} Hz")
print(f"  percent increase: {best['pct_increase']:.1f}%")
print(f"{'='*60}")


# ── build per-trial matrix for best unit ────────────────────────────────────
fr_row = best["fr_row"]      # (n_LV_samples,) for this session
trial_arrays = []
for trial in best["trials"]:
    idx  = trial["global_idx"]
    ts_s = trial["ts_s"]
    win  = (ts_s >= 0.0) & (ts_s <= 40.0)
    trial_fr = fr_row[idx[win]]
    trial_arrays.append(trial_fr)
...
title = (
    f"Firing rate — best unit: session {best['exp_date']}, unit {best['unit_id']}\n"
    f"Control (0-10 s) max: {best['mean_max_control']:.1f} Hz  |  "
    f"Odor (10-20 s) max: {best['mean_max_odor']:.1f} Hz  |  "
    f"+{best['pct_increase']:.1f}%  (n={n_trials} trials)"
)
...
ax1.axvspan(CONTROL_S[0], CONTROL_S[1], alpha=0.07, color="cyan",
            label="control window")
ax1.axvspan(ODOR_S[0],    ODOR_S[1],    alpha=0.07, color="yellow",
            label="odor window")
ax1.legend(fontsize=6, loc="upper right", framealpha=0.6)

# annotate session + unit in top-left corner of heatmap
ax1.text(0.01, 0.97,
...
out_path = os.path.join(OUT_DIR, "FR_best_unit_waterfall.png")
